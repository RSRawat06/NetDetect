...
def test_segment_by_flows():
  new_X, flow_metadata = segment.segment_by_flows(X_flat, metadata, 4)
  assert(new_X.shape == (396, 4, 4))
  assert(np.all(new_X == [x + [[0] * 4] for x in X_struct]))

  new_X, flow_metadata = segment.segment_by_flows(X_flat, metadata, 3)
  assert(new_X.shape == (396, 3, 4))
  assert(np.all(new_X == X_struct))

  new_X, flow_metadata = segment.segment_by_flows(X_flat, metadata, 2)
...
def test_segment_flows():
  ...
  X, Y = segment.segment_by_ips(new_X, flow_metadata, 5)
