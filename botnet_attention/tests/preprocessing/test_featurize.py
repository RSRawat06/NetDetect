...
def test_store_categorical_over():
  ...
  for i, row in enumerate([raw_first, raw_second]):
    if i == 1:
      failed = False
      try:
        all_records = featurize.store_categoricals(row, headers_key, all_records, ['proto'], ['type'], ['port'], 4)
      except ValueError:
        failed = True
      assert(failed)
      continue
    featurize.store_categoricals(row, headers_key, all_records, ['proto'], ['type'], ['port'], 4)
