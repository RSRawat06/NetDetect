...
class Attention_Discriminator():
  ...
  def build_model(self):
    self.x = tf.placeholder(tf.float32, [self.config['BATCH_S'], self.config['MAX_SEQUENCE_LENGTH'], self.config['MAX_PACKET_SEQUENCE_LENGTH'], self.config['N_FEATURES']], name="horizontal")
    self.target = tf.placeholder(tf.float32, [self.config['BATCH_S'], 2], name="target")
    # x.shape = [batch, flow, packet, input_feat]
    # y.shape = [batch, flow, packet, input_feat]
    # target.shape = [batch, 2]

    with tf.variable_scope("encode_x_flow"):
      self.fwd_lstm = tf.nn.rnn_cell.BasicLSTMCell(self.config['N_HIDDEN'], state_is_tuple=True)
      self.x_flow_output, self.x_flow_state = tf.nn.dynamic_rnn(cell=self.fwd_lstm, inputs=self.x, dtype=tf.float32)
      # x_flow_output.shape = [batch, flow, packet, output_feat]
      # x_flow_state.shape = [batch, flow, output_feat]

    with tf.variable_scope("encode_x"):
      self.second_fwd_lstm = tf.nn.rnn_cell.BasicLSTMCell(self.config['N_HIDDEN'], state_is_tuple=True)
      self.x_output, self.x_state = tf.nn.dynamic_rnn(cell=self.second_fwd_lstm, inputs=self.x_flow_state, dtype=tf.float32)
      # x_output.shape = [batch, flow, output_feat]
