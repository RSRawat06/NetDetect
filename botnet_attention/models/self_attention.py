...
class Self_Attention(Base_Model):
  ...
  def build_model(self):
    ...
    self.r_p = tf.nn.softmax(tf.matmul(self.W_s_2_p, tf.tanh(tf.matmul(self.W_s_1_p, tf.tranpose(self.H_p, (0, 2, 1))))))
    assert(self.r_p.shape == (self.config.BATCH_SIZE, self.config.N_FLOWS, self.config.N_PACKETS))

    self.M_p = tf.squeeze(tf.matmul(tf.transpose(self.H_p, (0, 2, 1)), tf.expand_dims(self.r_p, 3)))
